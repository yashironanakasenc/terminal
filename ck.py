...
This version works in non-interactive terminals by reading credentials from environment variables:
  IG_USERNAME
  IG_PASSWORD
Autosaves output files using the detected Instagram username.
"""
...
def convert_instagrapi_settings_to_playwright(insta_settings_path: str, playwright_path: str):
    """Convert instagrapi session JSON to Playwright storage state JSON."""
    ...
    cookie_source = data["cookies"] if isinstance(data.get("cookies"), dict) and data.get("cookies") else auth
...
def instagrapi_login_and_export(identifier: str, password: str):
    """Login to Instagram and export session + Playwright state."""
    cl = Client()
    ...
    try:
        ...
        raise RuntimeError("2FA required. This script does not auto-handle 2FA.")
    except ChallengeRequired:
        raise RuntimeError("Challenge required (email/phone verification). Resolve manually.")
...
def main():
    ...
    if not identifier or not password:
        ...
        print("Set these variables and re-run the script.")
        return
